...
export default function TaskCard({ task, onDelete, onEdit, onStatusChange }) {
  ...
  const formatDate = (dateStr) => {
    if (!dateStr) return '';
    // Parse YYYY-MM-DD as local date to avoid UTC midnight shifting for negative offsets
      ...
      <div className="task-card-header">
        ...
        <div className="task-actions">
          ...
          {!confirmingDelete ? (
            ...
            <>
              <button className="action-btn delete" title="Confirm Delete" onClick={() => onDelete(task.id)}>Confirm</button>
              <button className="action-btn" title="Cancel Delete" onClick={() => setConfirmingDelete(false)}>Cancel</button>
            </>
          )}
        </div>
      </div>
